...
test("serves the application shell", async () => {
  ...
  assert.match(html, /id="navMentorInstall"/);
  assert.match(html, /id="mobileMentorInstall"/);
  assert.match(html, /id="mentorInstallModal"/);
  assert.match(html, /assets\/foraldramentorer-logo\.svg/);
...
test("serves application assets and returns 404 for unknown files", async () => {
  ...
  assert.equal(manifest.start_url, "/#/mentor-home");
  assert.equal(manifest.icons.length, 2);
  const iconResponse = await worker.fetch(new Request("https://example.test/assets/icon-192.png"), {}, context);
  ...
  assert.match(await serviceWorker.text(), /foraldramentorer-mentor-v134-meeting-queue-current-data/);
